...
- Map every major TIL outcome to `implement`, `test`, `debug`, `interpret`, or `design`. Use one `practice/<area>/<topic>.ipynb` for foundational mathematics, small deterministic NumPy work, or self-contained Tensor/Shape mechanics when a reusable module boundary is not being learned. Use `practice/<area>/<topic>/workbook.ipynb` plus learner-owned `src/` stubs and normal/edge/failure `tests/` when reusable APIs, multiple modules or classes, isolated pytest/CI behavior, training jobs, or systems boundaries materially belong to the outcome.
...
- Short code may stay in a TIL. Use `.ipynb` alone for compact calculation, foundational NumPy implementation, or self-contained Tensor/Shape mechanics; use a Notebook-led `src/` and `tests/` bundle when reusable implementation boundaries, isolated testing, training jobs, or systems behavior matter.
