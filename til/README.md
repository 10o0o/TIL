...
강의자료를 보고 공부한 날에는 최종 TIL의 `관련 기록`에 실제 자료 링크를 남깁니다. 이 날짜별 TIL이 이후 실습의 시작점이 되므로 `$suggest-learning-practice`를 사용할 때는 생성된 `til/YYYY/MM/YYYY-MM-DD.md` 경로를 정확히 지정합니다. 스킬은 그 링크를 따라 강의자료를 확인하며, `til/today.md`나 자동으로 고른 최신 TIL은 입력으로 사용하지 않습니다.
